...
def get_product_info(slug):
    with semaphore:
        url = f"https://piloterr.com/api/v2/walmart/product?query={slug}"
...
# Update the JSON file with the product information
def update_json(slug, product_info):
...
def process_slugs(rows):
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {executor.submit(get_product_info, row[0]): row[0] for row in rows}
...
if __name__ == "__main__":
    ...
    # Read the CSV file with product slugs
